...
- `frost_vertex`：把全屏四边形射影成梯形。**只有远端那对顶点会动**（`anchor = 0` 时是上面两个角），
  近端两个角钉死。顶点输出齐次坐标的分子与 `w`，让 Metal 的透视校正插值对整个四边形使用同一个
  homography；画面坐标 0…1 铺满梯形，**是射影拉伸，不是裁切**。
- `frost_vertex_flat`：同样的四边形但不带梯形。拷贝那一趟必须用它（见踩坑）。
- `frost_copy`：捕获帧 1:1 写进工作纹理的第 0 级 mip。坐标取自 `[[position]]` 而不是插值 uv。
- `frost_fragment`：模糊。

```
motion = smoothstep(0, 1, 进度)
ramp   = (1 - uv.y) ^ 渐变曲线       // 顶端 1，铰链端 0；anchor = 1 时反过来
radius = 顶端最强模糊 × motion × ramp
```

5×5 加权抽头核，权重 `1 : 4 : 6 : 4 : 1`（归一化 256），**抽头间距就是这一行的半径**。图片覆盖率和
颜色一起进核，所以模糊会跨过梯形边界散射进外面的黑色；顶部每行的左、中、右都必须同时变糊。每个抽头去
...
| 顶端模糊 | 0 | 0 | 2 pt | 19 | 36 | 53 | 67 | **72 pt** |
...
MACDUO_PROBE_FLASH=1   ./.scratch/out/Products/Release/MacDuoProbe   # 屏幕上真显示 3 秒
...
| project | 纵向灰度斜坡 | 正向与镜像都只有一个 homography，无对角线接缝 |
| preview | 仿桌面 | 五张预览 PNG |

实测（M1 Pro，1280×800 合成帧，顶端收窄 0.35、模糊 72 pt、渐变 1.35）：
...
blur sigma: 51.7→38.2 · 35.1→35.5 · 19.7→20.3 · 7.5→7.8 · 0.2→0.0（最后一行保留 100% 细节）
full-width: 顶部左 / 中 / 右保留 0.1% / 0.0% / 0.1% 高频细节
composite:  3.2 ms/frame @3456×2234（每像素 25 次采样）
```
...
- **梯形轮廓对，不等于 homography 对**。若顶点只写 `float4(x * scale, y, 0, 1)`，Metal 会在两只三角形内
  分别仿射插值 UV；中间对角线上的导数和模糊足迹会接不上，内容也不符合固定观察点看平面的射影。
  必须让 clip-space `w` 随远近边变化。探针用纵向灰度斜坡同时检查正向和镜像的反射影公式，不能只测白色梯形轮廓。
- **模糊图片不模糊覆盖率，边缘就会像被剪刀切掉**。合成四边形必须按最外层抽头扩大绘制域，每个抽头将
  `texture(clamp(uv)) × coverage(uv)` 一起累加。否则梯形内部会糊，外轮廓却始终硬，与参考效果的黑色边缘散射不符。
